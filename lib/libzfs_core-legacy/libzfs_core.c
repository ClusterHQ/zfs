    ...
    nvlist_t *config, nvlist_t **resultp)
{
	...
	if (config) {
		packed_config = fnvlist_pack(source, &config_size);
...
{
	return (lzc_ioctl_simple(ZFS_IOC_POOL_STATS, pool, NULL, stats));
}
...
lzc_promote(const char *fsname, nvlist_t *opts, nvlist_t **outnvl)
{
	...
	strlcpy(name, fsname, sizeof (name));
	error = lzc_ioctl_impl(ZFS_IOC_CLONE, name, origin, &conflsnap, NULL,
    ...
    nvlist_t **errlist)
{
	...
	args = fnvlist_alloc();
	fnvlist_add_nvlist(args, "snaps", holds);

	if (cleanup_fd != -1)
		fnvlist_add_int32(args, "cleanup_fd", cleanup_fd);

	error = (lzc_ioctl_simple(ZFS_IOC_HOLD, pool, holds, errlist));
...
lzc_list_find_children(const char *name, lzc_iter_f cb,
    const dmu_objset_stats_t *top_stats, void *data, uint64_t mindepth,
    uint64_t maxdepth, int flags)
{
	nvlist_t *outnvl;
	uint64_t cookie;
	int err = 0;
	...
	dmu_objset_stats_t objset_stats;
	char fsname[MAXNAMELEN];
	boolean_t allocated_stats = B_FALSE;

	strlcpy(fsname, name, sizeof (fsname));

	if (((flags & DLS_IGNORE_TOP) == 0) &&
	    (flags & DLS_TRAVERSE_SNAPSHOT) == 0 &&
	    (flags & DLS_RECURSE) != 0 &&
	    (flags & DLS_IGNORE_LISTSNAPS) == 0) {
		nvlist_t *props, *nvl;
		uint64_t listsnap;
		if ((err = lzc_pool_getprops(name, NULL, &props)) != 0)
			return (err);

		if (nvlist_lookup_nvlist(props,
		    zpool_prop_to_name(ZPOOL_PROP_LISTSNAPS), &nvl) != 0 &&
		    nvlist_lookup_uint64(nvl, ZPROP_VALUE, &listsnap)) {
	...
	if (maxdepth) {
		...
		while ((err = lzc_ioctl_impl(ZFS_IOC_DATASET_LIST_NEXT, fsname,
			...
			if (flags & DLS_RECURSE) {
				err = lzc_list_find_children(fsname, cb,
				    &objset_stats, data,
				    min, max, flags | DLS_IGNORE_TOP);
			}

			if (!((objset_stats.dds_type == DMU_OST_ZFS) &&
			    (flags & DLS_TRAVERSE_FILESYSTEM) != 0) &&
			    !((objset_stats.dds_type == DMU_OST_ZVOL) && (flags
			...
			nvl = dmu_objset_stats_nvlist(&objset_stats);
	...
	if (err == 0 && top_stats == NULL) {
		top_stats = malloc(sizeof (dmu_objset_stats_t));
		allocated_stats = B_TRUE;
		strlcpy(fsname, name, sizeof (fsname));
		if ((err = lzc_ioctl_impl(ZFS_IOC_OBJSET_STATS, fsname, NULL,
		    NULL, NULL, NULL, 0, (dmu_objset_stats_t *)top_stats, NULL,
		    NULL, &props))) {
			free((void *)top_stats);
			return (err);
		}
	...
	if (err == 0 && mindepth < 2 && flags & DLS_TRAVERSE_SNAPSHOT) {
		nvlist_t *props;
		cookie = 0;
		strlcpy(fsname, name, sizeof (fsname));
	...
	if (err == 0 && mindepth < 2 && flags & DLS_TRAVERSE_BOOKMARK) {
		nvpair_t *pair;
		nvlist_t *bmarks;
		...
		for (pair = nvlist_next_nvpair(bmarks, NULL);
			...
			fnvlist_add_string(outnvl, "name", fsname);
			fnvlist_add_nvlist(outnvl, "properties", bmark_props);

			nvl = dmu_objset_stats_nvlist(top_stats);
			fnvlist_add_nvlist(outnvl, "dmu_objset_stats", nvl);
	...
	if (err == 0 && (flags & DLS_IGNORE_TOP) == 0 && mindepth == 0) {
		nvlist_t *nvl;
		outnvl = fnvlist_alloc();

		fnvlist_add_string(outnvl, "name", name);
		fnvlist_add_nvlist(outnvl, "properties", props);
...
lzc_list_worker(void *args)
{
	...
	if (name) {
		zpr.zpr_err = (uint8_t)lzc_list_find_children(name,
		    lctxp->lctx_func, NULL, lctxp->lctx_data, mindepth,
		    maxdepth, flags);
	} else {
		nvlist_t *config;
		nvpair_t *elem = NULL;
